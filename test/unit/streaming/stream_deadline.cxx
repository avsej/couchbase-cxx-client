...
void
a_clean_end_wins_over_a_close_recorded_during_the_read([[maybe_unused]] context& ctx)
{
  asio::io_context io;
  ...
  // http_session runs this from inside the read completion that finished the response, after the
  // connection has gone back to the keep-alive pool and before the body learns the read is done.
  // A deadline taken here has the body still looking mid-stream, so it records a terminal over a
  // response the server had already completed.
  auto on_stream_end = [&io, body]() {
    ...
    assert_eq(body->value().set_deadline(std::chrono::steady_clock::now() - 1s,
                                         couchbase::core::io::deadline_terminal::unambiguous),
              couchbase::core::io::deadline_state::armed,
              "the body is still live inside the read completion that finished it");
    // Nested drain on the same thread, so the expiry lands before this handler returns.
    io.poll();
  };
  ...
  // The terminal the clean end won has to hold for every later pull too. A body that delivered the
  // response and then reported the superseded timeout would contradict itself one call apart.
...
tests() -> test_suite
{
  return {
    suite_name,
    {
      ...
      { CASE(a_clean_end_wins_over_a_close_recorded_during_the_read) },
