...
{
  // The state a bucket is in while its replica count is being reduced: the lower
  // num_replicas is published at once and the wider rows stay until a rebalance.
  // Bounding on the row alone would read a copy the bucket no longer keeps.
  requires_error(resolve(/* map = */ vbucket_map{ { 0, 1, 2 } },
                         /* num_replicas = */ 1,
                         replica_index::second,
                         /* wrap = */ false),
