...
//
// This also costs real wall-clock time, which is why the wait for the child below has no
// deadline of its own. cmake/Testing.cmake passes --leak-check=full --show-reachable=yes
...
// for the next four minutes while valgrind wrote that report, and a four-minute deadline
// here killed it and reported a wedge that had not happened. Raising it to six minutes only
// moved the false failure to a 391-second run.
//
...
TEST_CASE("integration: cluster remains usable in a forked child", "[integration]")
{
  ...
  // Wait for the child without a deadline of our own, and let CTest's timeout be the
  // backstop against a genuinely stuck child. Retry on EINTR; anything else is a real
  // waitpid() failure and is reported below.
  //
  // This deliberately does NOT impose a per-test deadline, and that is a reversal: two were
  // tried and both produced false failures. The quantity such a deadline has to bound is not
  // how long the child takes to do its work -- it finishes that in seconds -- but how long
  // valgrind takes to write its exit-time leak report for a heap that _Exit() leaves dirty
  // (see the note at the top of this file). Measured, that is 33-46s on a developer box,
  // over 236s in one CI run, and over 391s in another, on the same code. A four-minute bound
  // killed a child that had already succeeded; a six-minute bound did the same. There is no
  // constant here that is both large enough never to lie and small enough to be useful, so
  // the honest thing is not to guess one: a slow child makes this test slow, and a hung child
  // is CTest's to time out.
  int status{};
  pid_t reaped{ -1 };
  do {
    reaped = waitpid(child_pid, &status, 0);
