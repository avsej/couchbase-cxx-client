...
class cluster::impl : public std::enable_shared_from_this<cluster::impl>
{
  public:
    explicit impl(asio::io_context& ctx)
      : ctx_(ctx)
    ...
    void open(const couchbase::origin& origin, std::function<void(std::error_code)>&& handler)
    {
        ...
        if (origin_.options().enable_dns_srv) {
            return asio::post(asio::bind_executor(
              ctx_, [self = shared_from_this(), handler = std::move(handler)]() mutable { return self->do_dns_srv(std::move(handler)); }));
        }
    ...
    {
        do_ping(std::move(report_id), std::move(bucket_name), std::move(services), std::move(handler));
    }
...
void
cluster::open(const origin& origin, std::function<void(std::error_code)>&& handler)
{
    return impl_->open(origin, std::move(handler));
}

void
cluster::close(std::function<void()>&& handler)
{
    return impl_->close(std::move(handler));
}

void
cluster::open_bucket(const std::string& bucket_name, std::function<void(std::error_code)>&& handler)
{
    return impl_->open_bucket(bucket_name, std::move(handler));
}

void
cluster::close_bucket(const std::string& bucket_name, std::function<void(std::error_code)>&& handler)
{
    return impl_->close_bucket(bucket_name, std::move(handler));
}

void
cluster::diagnostics(std::optional<std::string> report_id, std::function<void(couchbase::diag::diagnostics_result)>&& handler)
{
    return impl_->diagnostics(report_id, std::move(handler));
}
...
{
    return impl_->ping(report_id, bucket_name, services, std::move(handler));
}
