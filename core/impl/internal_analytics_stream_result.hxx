...
public:
  ...
  /**
   * Returns the analytics signature captured during preamble parsing.
   */
...
private:
  ...
  std::shared_ptr<std::promise<std::pair<error, analytics_meta_data>>> meta_promise_{
    std::make_shared<std::promise<std::pair<error, analytics_meta_data>>>()
  };
};
} // namespace couchbase
