...
{
public:
  explicit internal_query_stream_result(core::query_stream stream);
  ...
  /**
   * Returns the query signature captured during preamble parsing.
   */
...
private:
  ...
  std::shared_ptr<std::promise<std::pair<error, query_meta_data>>> meta_promise_{
    std::make_shared<std::promise<std::pair<error, query_meta_data>>>()
  };
};
} // namespace couchbase
