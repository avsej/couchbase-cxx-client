                 ...
                 const jsonsl_char_t* /* at */)
{
  ...
  if (impl->emit_next_row_) {
    auto row = impl->get_buffer_region(
      state->pos_begin, lexer->pos - state->pos_begin + (state->type == JSONSL_T_SPECIAL ? 0 : 1));
                            ...
                            const jsonsl_char_t* at)
{
  ...
  if (state->type == JSONSL_T_HKEY) {
    impl->last_key_ =
      impl->buffer_.substr(state->pos_begin + 1, state->pos_cur - state->pos_begin - 1);
  }
                                       ...
                                       std::size_t max_buffer)
{
  ...
  if (error != JSONSL_ERROR_SUCCESS) {
    throw std::invalid_argument(std::string("unable to create JSON pointer: ") +
...
streaming_lexer::feed(std::string_view data)
{
  ...
  /* Enforce single-row / trailer buffer ceiling BEFORE feeding, so a huge
   * chunk that has not yet been parsed does not OOM the process. The
   * un-consumed portion is everything buffered since the last keep_position_
   * trim, i.e. buffer_.size() minus the bytes already consumed.
   *
   * TODO: meta_buffer_ (trailing metadata) is not bounded by max_buffer_;
   * guard if a huge trailer becomes a concern.
   */
  if (impl_->max_buffer_ != 0) {
    ...
    if (unconsumed > impl_->max_buffer_) {
      impl_->aborted_ = true;
...
void
streaming_lexer::on_complete(
  std::function<void(std::error_code, std::size_t, std::string&&)> handler)
{
