...
{
static inline const std::array<std::uint32_t, 256> crc32tab = {
...
hash_crc32(Iterator begin, Iterator end)
{
    std::uint32_t crc = UINT32_MAX;

    for (Iterator it = begin; it != end; ++it) {
        crc = (crc >> 8) ^ crc32tab[(crc ^ static_cast<std::uint64_t>(*it)) & 0xff];
    }
...
constexpr std::uint32_t
hash_crc32(std::string_view key)
{
    return hash_crc32(key.begin(), key.end());
}
