 ...
 */
template<class T>
typename std::enable_if_t<std::is_unsigned_v<T>, std::pair<T, std::vector<std::byte>>>
decode_unsigned_leb128(const std::vector<std::byte>& buf, leb_128_no_throw /* unused */)
{
    ...
    if ((buf[0] & std::byte{ 0b1000'0000 }) == std::byte{ 0b1000'0000 }) {
        ...
        if (end == buf.size()) {
            return { 0U, std::vector<std::byte>{} };
        }
    }
    // Return the decoded value and a buffer for any remaining data
    return { rv, std::vector<std::byte>{ buf.begin() + static_cast<std::vector<std::byte>::difference_type>(end) + 1, buf.end() } };
}
...
template<class T>
typename std::enable_if_t<std::is_unsigned_v<T>, std::pair<T, std::vector<std::byte>>>
decode_unsigned_leb128(const std::vector<std::byte>& buf)
{
    if (!buf.empty()) {
 ...
 */
template<class T>
typename std::enable_if_t<std::is_unsigned_v<T>, std::vector<std::byte>>
skip_unsigned_leb128(const std::vector<std::byte>& buf)
{
