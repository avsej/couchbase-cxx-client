...
transaction_get_result::create_from(const core::operations::lookup_in_response& resp)
{
    ...
    if (resp.fields[6].status == key_value_status_code::success) {
        forward_compat = codec::tao_json_serializer::deserialize<tao::json::value>(resp.fields[6].value);
    }
...
transaction_get_result
transaction_get_result::create_from(const transaction_get_result& document, codec::encoded_value content)
{
    return {
        document.id(),
        std::move(content),
