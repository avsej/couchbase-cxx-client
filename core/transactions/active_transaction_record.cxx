...
process_document_ids(const tao::json::value& entry, const std::string& key)
{
...
map_to_atr(const operations::lookup_in_response& resp)
{
    ...
    if (resp.fields[0].status == key_value_status_code::success) {
        ...
        for (const auto& [key, val] : attempts.get_object()) {
            ...
            std::optional<std::uint32_t> expires_after_msec = std::max(val.optional<int32_t>(ATR_FIELD_EXPIRES_AFTER_MSECS).value_or(0), 0);
...
void
active_transaction_record::get_atr(const core::cluster& cluster,
