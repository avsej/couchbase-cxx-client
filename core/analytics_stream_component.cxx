                               ...
                               std::chrono::milliseconds timeout) -> std::string
{
  ...
  if (request.scope_qualifier) {
    ...
    body["query_context"] =
      fmt::format("default:`{}`.`{}`", request.bucket_name.value(), request.scope_name.value());
  }
               ...
               analytics_stream_component::handler_type&& handler)
  {
    ...
    auto op = http_.do_http_request(
      ...
      [self = shared_from_this(), callback_state](http_response resp, dispatch_error err) mutable {
        ...
        auto stream = std::make_shared<analytics_stream>(self->io_, resp.body());
...
analytics_stream_component::analytics_stream_component(asio::io_context& io,
                                                       http_component http,
                                                       std::chrono::milliseconds default_timeout)
  : impl_{ std::make_shared<analytics_stream_component_impl>(io, std::move(http), default_timeout) }
{
}
