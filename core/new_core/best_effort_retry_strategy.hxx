...
{
  public:
    explicit best_effort_retry_strategy(backoff_calculator calculator = controlled_backoff);
    ~best_effort_retry_strategy() override = default;

    auto retry_after(std::shared_ptr<retry_request> request, retry_reason reason) -> retry_action override;
