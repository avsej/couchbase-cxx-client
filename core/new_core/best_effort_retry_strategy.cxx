...
#include "best_effort_retry_strategy.hxx"
...
exponential_backoff(std::chrono::milliseconds min_backoff, std::chrono::milliseconds max_backoff, double backoff_factor)
  -> backoff_calculator
{
    double min = 1'000'000;   // 1 millisecond
    double max = 500'000'000; // 500 milliseconds
    double factor = 2;
...
auto
best_effort_retry_strategy::retry_after(std::shared_ptr<retry_request> request, retry_reason reason) -> retry_action
{
