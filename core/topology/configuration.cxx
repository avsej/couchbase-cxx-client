...
parse_configuration(std::string_view input, const endpoint& source) -> configuration
{
    ...
    auto v = parser.parse(input);

    result.id = couchbase::core::uuid::random();
    result.epoch = v["epoch"];
    result.rev = v["rev"];
    if (auto node_locator = v.at_key("nodeLocator"); node_locator.is_string()) {
        if (node_locator.get_string().value() == "ketama") {
    ...
    if (!has_this_node) {
        for (auto& node : result.nodes) {
            ...
            if (node.hostname == source.address && (kv_port == source.port || kv_tls_port == source.port)) {
                node.this_node = true;
                break;
            }
        }
