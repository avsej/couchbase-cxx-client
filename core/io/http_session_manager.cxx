...
{
static auto
split_host_port(const std::string& address) -> std::pair<std::string, std::uint16_t>
{
...
class http_session_manager_impl : public std::enable_shared_from_this<http_session_manager_impl>
{
  public:
    http_session_manager_impl(std::string client_id, asio::io_context& ctx, asio::ssl::context& tls)
    ...
    {
        const std::scoped_lock config_lock(config_mutex_, sessions_mutex_);
        config_ = config;
    ...
    void check_in(service_type type, std::shared_ptr<http_session> session)
    {
        ...
        if (!session->is_stopped()) {
            ...
            std::scoped_lock lock(sessions_mutex_);
            idle_sessions_[type].emplace_back(session);
...
http_session_manager::http_session_manager(std::string client_id, asio::io_context& ctx, asio::ssl::context& tls)
  : impl_{ std::make_shared<http_session_manager_impl>(std::move(client_id), ctx, tls) }
{
}

void
http_session_manager::update_config(const topology::configuration& config)
{
    return impl_->update_config(config);
}

void
http_session_manager::set_configuration(const topology::configuration& config, const cluster_options& options)
{
    return impl_->set_configuration(config, options);
}

void
http_session_manager::set_tracer(std::shared_ptr<couchbase::tracing::request_tracer> tracer)
{
    return impl_->set_tracer(std::move(tracer));
}

void
http_session_manager::set_meter(std::shared_ptr<couchbase::metrics::meter> meter)
{
    return impl_->set_meter(std::move(meter));
}
...
void
http_session_manager::export_diag_info(diag::diagnostics_result& res)
{
