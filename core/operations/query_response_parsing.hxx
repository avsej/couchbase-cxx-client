...
/**
 * Map query meta-data and HTTP status to an error_code.
 *
 * Pure classifier — no side effects, no cache mutations, no exceptions thrown.
 * Codes 4040/4050/4070 are classified as prepared_statement_failure here;
 * the cache-erase + retry_http_request throw for those codes remains in make_response.
 *
 * Returns an empty error_code when the query succeeded (meta.status == "success" and no
 * prepared-statement retry needed).
 */
auto
map_query_error(const query_response::query_meta_data& meta,
                std::uint32_t http_status) -> std::error_code;
