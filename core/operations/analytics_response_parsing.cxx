...
map_analytics_error(const analytics_response::analytics_meta_data& meta) -> std::error_code
{
  ...
  if (meta.errors.empty()) {
    /* Unreachable from make_response, which only reaches the error branch after the server
     * reports a non-success status with at least one error; present for the streaming-path
     * caller, which has no such guarantee. */
