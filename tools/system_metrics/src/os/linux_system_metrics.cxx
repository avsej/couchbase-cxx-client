...
load_file_impl(const std::string& name) -> std::string
{
  ...
  if (!file.is_open()) {
    ...
    throw std::system_error(errno, std::system_category(), "load_file_impl(" + name + ") failed");
  }
...
auto
linux_system_metrics::get_system_boot_time() const -> std::uint64_t
{
  return SystemConstants::instance().boot_time;
}

auto
linux_system_metrics::get_logical_cpu_count() const -> std::uint32_t
{
...
auto
linux_system_metrics::get_proc_cpu(sm_pid_t pid) const -> proc_cpu_t
{
...
auto
linux_system_metrics::get_proc_memory(sm_pid_t pid) const -> proc_mem_t
{
  const auto pstat = proc_stat_read(pid);
...
void
linux_system_metrics::iterate_proc_threads(sm_pid_t pid, IterateThreadCallback callback)
{
