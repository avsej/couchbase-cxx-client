...
auto
windows_system_metrics::get_system_boot_time() const -> std::uint64_t
{
  return SystemConstants::instance().boot_time;
}

auto
windows_system_metrics::get_logical_cpu_count() const -> std::uint32_t
{
...
auto
windows_system_metrics::get_proc_cpu(sm_pid_t pid) const -> proc_cpu_t
{
  HANDLE hProcess =
...
auto
windows_system_metrics::get_proc_memory(sm_pid_t pid) const -> proc_mem_t
{
  proc_mem_t procmem{};
...
void
windows_system_metrics::iterate_proc_threads(sm_pid_t pid, IterateThreadCallback callback)
{
  HANDLE hProcess =
    OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, static_cast<DWORD>(pid));
  if (hProcess == NULL) {
