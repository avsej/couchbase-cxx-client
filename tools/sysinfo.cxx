...
struct system_info {
  ...
  [[nodiscard]] auto threads_string() const -> std::string
  {
    ...
    for (const auto& thread : threads) {
      fmt::format_to(std::back_inserter(buffer),
                     "  {:>8d}  {:<20s}  {:<20s}\n",
                     thread.tid,
                     human_readable_duration(std::chrono::microseconds{ thread.utime }),
                     human_readable_duration(std::chrono::microseconds{ thread.stime }));
    }
  ...
  [[nodiscard]] auto cpu_string() const -> std::string
  {
    ...
    std::chrono::milliseconds start_time_ms{ cpu.start_time };
    ...
    fmt::format_to(std::back_inserter(buffer),
                   ...
                   start_time_ms);
    fmt::format_to(
      std::back_inserter(buffer), "  System: {}\n", std::chrono::milliseconds{ cpu.sys });
    fmt::format_to(
      std::back_inserter(buffer), "  User:   {}\n", std::chrono::milliseconds{ cpu.user });
  ...
  {
    std::chrono::seconds boot_time_s{ system_boot_time };
...
public:
  ...
  void execute() const
  {
    ...
    do {
      ...
      info.pid = static_cast<system_metrics::sm_pid_t>(pid_);
