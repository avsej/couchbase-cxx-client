...
// Fetch the bucket's current topology configuration (blocking).
auto
fetch_bucket_configuration(const couchbase::cluster& cluster, const std::string& bucket_name)
...
auto
bucket_kv_ready(const couchbase::cluster& cluster, const std::string& bucket_name) -> bool
{
  const auto config = fetch_bucket_configuration(cluster, bucket_name);
...
auto
make_ping_options(const options& opts) -> couchbase::ping_options
{
...
wait_until_ready(const couchbase::cluster& cluster, const options& opts) -> std::error_code
{
  ...
  const auto ping_opts = make_ping_options(opts);
  for (;;) {
    auto [err, report] = cluster.ping(ping_opts).get();
                 ...
                 const options& opts) -> std::error_code
{
  ...
  for (;;) {
    ...
    if (bucket_kv_ready(cluster, bucket_name)) {
      auto [err, report] = bucket.ping(ping_opts).get();
      if (!err.ec() && ping_predicate_satisfied(report, opts.desired_state, opts.service_types)) {
        return {};
      }
    }
