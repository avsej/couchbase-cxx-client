...
#include "tools/severity_filtering_processor.hxx"
#include "utils.hxx"
...
private:
  ...
  {
    fmt::println(stderr, "init_logger");
    ...
    if (use_http_logger_) {
      ...
      fmt::println(stderr, "use couchbase::observability::fmt_log_exporter");
      auto exporter = std::make_unique<couchbase::observability::fmt_log_exporter>(stderr);
      auto processor = std::make_unique<couchbase::observability::severity_filtering_processor>(
        sdk::logs::SimpleLogRecordProcessorFactory::Create(std::move(exporter)),
        logs::Severity::kTrace);
      std::vector<std::unique_ptr<sdk::logs::LogRecordProcessor>> processors;
      processors.emplace_back(std::move(processor));
      ...
      CB_LOG_DEBUG("Test Debug",
                   opentelemetry::common::MakeAttributes({
                     { "custom_property", "hello, world" },
                   }));

      CB_LOG_CRITICAL("Test Debug",
                      opentelemetry::common::MakeAttributes({
                        { "custom_property", "hello, world" },
                      }));
    }
