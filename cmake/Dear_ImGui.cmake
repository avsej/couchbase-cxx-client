...
# =================================
#
# Build instructions: 1. Install latest CMake * Windows: https://cmake.org/download/ (Tick checkbox to place cmake in
# system PATH) * Linux: from your favorite package manager * MacOS: brew install cmake 2. Open command prompt in
# directory containing "imgui" and "imgui_dev" folders * Windows: open Visual Studio tools command prompt or specify -G
# "Generator Name" (quotes are important). * Other platforms: Specifying generator name is optional. 3. Create a build
# directory and configure build git clone https://github.com/ocornut/imgui.git mkdir cmake-build cd cmake-build cmake
# <optional build parameters> ../imgui 4. Build * Windows (Visual Studio generators): Open generated imgui_dev.sln * All
# platforms: cmake --build .
#
# Example of using Dear ImGui in your CMake project:
#
# # Set various library options: set(IMGUI_SDL_TARGET sdl CACHE STRING "" FORCE) # Include Dear ImGui into build.
# add_subdirectory(path/to/imgui) <..> # Link Dear ImGui to your application: #  imgui - a main library target #
# imgui-sdl2 - a platform backend target #  imgui-dx11 - a graphics backend target target_link_libraries(YourApplication
# PUBLIC imgui imgui-sdl2 imgui-dx11)
#
# A list of defined targets: imgui-sdl2: platform backend for Windows/Linux/MacOS/etc using SDL2. imgui-glfw: platform
# backend for Windows/Linux/MacOS/etc using GLFW. imgui-osx: platform backend for MacOS. imgui-win32: platform backend
# for Windows. imgui-glut: platform backend for Windows/Linux/MacOS using glut/freeglut with OpenGL. imgui-metal:
# graphics backend for MacOS using Metal graphics API. imgui-opengl3: graphics backend for Windows/Linux/MacOS/etc using
# OpenGL3 graphics API. imgui-opengl2: graphics backend for Windows/Linux/MacOS/etc using OpenGL2 graphics API.
# imgui-vulkan: graphics backend for Windows/Linux/MacOS/etc using Vulkan graphics API. imgui-dx9: graphics backend for
# Windows using DirectX 9 graphics API. imgui-dx10: graphics backend for Windows using DirectX 10 graphics API.
# imgui-dx11: graphics backend for Windows using DirectX 11 graphics API. imgui-dx12: graphics backend for Windows using
# DirectX 12 graphics API. imgui-sdlrenderer2: graphics backend for platforms supported by SDL.
#
...
set(IMGUI_SDL_TARGET
    ""
    CACHE STRING "A custom SDL target name that will be used for linking to examples.")
set(IMGUI_GLFW_TARGET
    ""
    CACHE STRING "A custom GLFW target name that will be used for linking to examples.")
set(IMGUI_GLUT_TARGET
    ""
    CACHE STRING "A custom GLFW target name that will be used for linking to examples.")
...
# Warnings
# ######################################################################################################################
function(imgui_set_compiler_warnings target)
  if(CMAKE_CXX_COMPILER_ID MATCHES "GNU|Clang")
    target_compile_options(
      ...
      -Wnarrowing
      -Wno-sign-conversion
      -Wno-error=declaration-after-statement) # $<$<COMPILE_LANGUAGE:CXX>:-pedantic>
    if(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
        target_compile_options(${target} INTERFACE -Wnontrivial-memaccess)
    endif()
    if(NOT APPLE)
      target_compile_options(${target} INTERFACE $<$<COMPILE_LANGUAGE:CXX>:-Wno-deprecated-copy>)
    endif()
  endif()

  # These warnings are overzealous in GCC and only practical to use with Clang.
  if(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
      target_compile_options(${target} INTERFACE -Wshadow)
  endif()
endfunction()
...
file(
  ...
  ${IMGUI_EXAMPLES_DIR}/libs/*
  imstb_*.c
  ../tracy/*.cpp)
set_source_files_properties(${THIRD_PARTY_SOURCE_FILES} PROPERTIES COMPILE_OPTIONS -w)

# if (CMAKE_CXX_COMPILER_ID MATCHES "GNU") add_compile_options(${imgui_target}
# $<$<COMPILE_LANGUAGE:CXX>:-Wno-class-memaccess>) endif ()

# ######################################################################################################################
# Main library
# ######################################################################################################################
# Set up main library. It is created as an INTERFACE library because it allows using different IMGUI_USER_CONFIG defines
# in different targets of downstream projects.
file(
  GLOB
  IMGUI_SOURCE_FILES
  ${IMGUI_ROOT_DIR}/*.h
...
imgui_set_compiler_warnings(imgui)
...
if(IMGUI_DISABLE_OBSOLETE_FUNCTIONS)
    target_compile_definitions(imgui PRIVATE -DIMGUI_DISABLE_OBSOLETE_FUNCTIONS=1)
endif()
if(IMGUI_DISABLE_OBSOLETE_KEYIO)
  target_compile_definitions(imgui PRIVATE -DIMGUI_DISABLE_OBSOLETE_KEYIO=1)
endif()
if(IMGUI_DISABLE_FILE_FUNCTIONS)
  target_compile_definitions(imgui PRIVATE -DIMGUI_DISABLE_FILE_FUNCTIONS=1)
endif()
if(IMGUI_USER_CONFIG)
  target_compile_definitions(imgui PRIVATE "-DIMGUI_USER_CONFIG=<${IMGUI_USER_CONFIG}>")
endif()
target_include_directories(imgui PRIVATE ${IMGUI_ROOT_DIR})
if(NOT IMGUI_DEMO)
    target_compile_definitions(imgui PRIVATE -DIMGUI_DISABLE_DEMO_WINDOWS=1)
endif()
if(IMGUI_ENABLE_STDLIB_SUPPORT)
    target_include_directories(imgui PRIVATE misc/cpp)
...
if(IMGUI_ENABLE_COVERAGE AND CMAKE_CXX_COMPILER_ID MATCHES "GNU")
    target_compile_options(imgui PRIVATE -fprofile-arcs -ftest-coverage)
    target_link_libraries(imgui PRIVATE -fprofile-arcs -ftest-coverage)
    target_link_libraries(imgui PRIVATE gcov)
endif()

if(IMGUI_ENABLE_ASAN AND CMAKE_CXX_COMPILER_ID MATCHES "GNU")
    target_compile_options(imgui PRIVATE -fsanitize=address -fno-omit-frame-pointer -fno-optimize-sibling-calls)
    target_link_libraries(imgui PRIVATE -g -fsanitize=address)
endif()
...
if(IMGUI_ENABLE_FREETYPE)
  ...
  imgui_set_compiler_warnings(imgui-freetype)
  target_sources(imgui-freetype PRIVATE ${IMGUI_MISC_DIR}/freetype/imgui_freetype.h
                                          ${IMGUI_MISC_DIR}/freetype/imgui_freetype.cpp)
target_include_directories(imgui-freetype PRIVATE ${IMGUI_ROOT_DIR})

  find_package(Freetype)
  if(FREETYPE_FOUND)
      target_include_directories(imgui-freetype PRIVATE ${FREETYPE_INCLUDE_DIRS})
      target_link_libraries(imgui-freetype PRIVATE ${FREETYPE_LIBRARIES})
  ...
  target_compile_definitions(imgui-freetype PRIVATE -DIMGUI_ENABLE_FREETYPE=1)
  target_link_libraries(imgui PRIVATE imgui-freetype)
endif()

# ######################################################################################################################
# C++ standard library
# ######################################################################################################################
add_library(imgui-stdlib STATIC)
imgui_set_compiler_warnings(imgui-stdlib)
target_sources(imgui-stdlib PRIVATE ${IMGUI_MISC_DIR}/cpp/imgui_stdlib.h ${IMGUI_MISC_DIR}/cpp/imgui_stdlib.cpp)
...
if(IMGUI_ENABLE_TRACY AND EXISTS ${IMGUI_ROOT_DIR}/../tracy)
  ...
  imgui_set_compiler_warnings(TracyLib)
...
if(IMGUI_IMPL_SDL)
  ...
  elseif(
    ...
    set_target_properties(
      SDL2::SDL2 PROPERTIES IMPORTED_LOCATION "$ENV{SDL2_DIR}/lib/${IMGUI_SDL_PLATFORM_ARCH}/SDL2.dll"
                            IMPORTED_IMPLIB "$ENV{SDL2_DIR}/lib/${IMGUI_SDL_PLATFORM_ARCH}/SDL2.lib")
  elseif(PkgConfig_FOUND)
    # Rest of the platforms (like MacOS) can consume SDL via pkg-config. CMake 3.6 supports IMPORTED_TARGET parameter
    # which creates PkgConfig::sdl2 target that we can easily link to. TODO: Switch to using IMPORTED_TARGET when CMake
    # minimal version is increased.
  ...
  if(TARGET ${IMGUI_SDL_TARGET} OR CMAKE_SYSTEM_NAME STREQUAL Emscripten)
    ...
    imgui_set_compiler_warnings(imgui-sdl2)
    target_sources(imgui-sdl2 INTERFACE ${IMGUI_BACKENDS_DIR}/imgui_impl_sdl2.h
    ...
    imgui_set_compiler_warnings(imgui-sdlrenderer2)
    target_sources(imgui-sdlrenderer2 INTERFACE ${IMGUI_BACKENDS_DIR}/imgui_impl_sdlrenderer2.h
...
if(IMGUI_IMPL_METAL AND APPLE)
  ...
  imgui_set_compiler_warnings(imgui-metal)
  target_sources(imgui-metal INTERFACE ${IMGUI_BACKENDS_DIR}/imgui_impl_metal.h
...
if(IMGUI_IMPL_OSX AND APPLE)
  ...
  imgui_set_compiler_warnings(imgui-osx)
  target_sources(imgui-osx INTERFACE ${IMGUI_BACKENDS_DIR}/imgui_impl_osx.h ${IMGUI_BACKENDS_DIR}/imgui_impl_osx.mm)
...
if(IMGUI_IMPL_WIN32 AND WIN32)
  ...
  imgui_set_compiler_warnings(imgui-win32)
  target_sources(imgui-win32 INTERFACE ${IMGUI_BACKENDS_DIR}/imgui_impl_win32.h
...
if(IMGUI_IMPL_GLFW)
  ...
  elseif(MSVC)
    ...
    target_link_libraries(
      glfw INTERFACE ${IMGUI_ROOT_DIR}/examples/libs/glfw/lib-vc2010-${IMGUI_GLFW_PLATFORM_BITS}/glfw3.lib)
  ...
  if(TARGET ${IMGUI_GLFW_TARGET})
    ...
    imgui_set_compiler_warnings(imgui-glfw)
    target_sources(imgui-glfw PRIVATE ${IMGUI_BACKENDS_DIR}/imgui_impl_glfw.h
                                        ${IMGUI_BACKENDS_DIR}/imgui_impl_glfw.cpp)
                                    target_link_libraries(imgui-glfw PRIVATE ${CMAKE_DL_LIBS} ${IMGUI_GLFW_TARGET})
                                    target_include_directories(imgui-glfw SYSTEM PRIVATE ${IMGUI_BACKENDS_DIR})
target_include_directories(imgui-glfw PRIVATE ${IMGUI_ROOT_DIR})
  else()
...
if(IMGUI_IMPL_OPENGL2 OR IMGUI_IMPL_OPENGL3)
  ...
  if(IMGUI_IMPL_OPENGL2)
    ...
    imgui_set_compiler_warnings(imgui-opengl2)
  ...
  if(IMGUI_IMPL_OPENGL3)
    ...
    imgui_set_compiler_warnings(imgui-opengl3)
    target_sources(imgui-opengl3 PRIVATE ${IMGUI_BACKENDS_DIR}/imgui_impl_opengl3.cpp)
    target_link_libraries(imgui-opengl3 PRIVATE imgui OpenGL::GL)
    target_include_directories(imgui-opengl3 SYSTEM PRIVATE ${IMGUI_BACKENDS_DIR})
target_include_directories(imgui-opengl3 PRIVATE ${IMGUI_ROOT_DIR})
  endif()
endif()
if(IMGUI_IMPL_GLUT)
  ...
  if(GLUT_glut_LIBRARY OR IMGUI_GLUT_TARGET)
    ...
    imgui_set_compiler_warnings(imgui-glut)
    target_sources(imgui-glut INTERFACE ${IMGUI_BACKENDS_DIR}/imgui_impl_glut.h
...
if(IMGUI_IMPL_DX9)
  ...
  if(HAS_D3D9)
    ...
    imgui_set_compiler_warnings(imgui-dx9)
    target_sources(imgui-dx9 INTERFACE ${IMGUI_BACKENDS_DIR}/imgui_impl_dx9.h ${IMGUI_BACKENDS_DIR}/imgui_impl_dx9.cpp)
...
if(IMGUI_IMPL_DX10)
  ...
  if(HAS_D3D10)
    ...
    imgui_set_compiler_warnings(imgui-dx10)
    target_sources(imgui-dx10 INTERFACE ${IMGUI_BACKENDS_DIR}/imgui_impl_dx10.h
...
if(IMGUI_IMPL_DX11)
  ...
  if(HAS_D3D11)
    ...
    imgui_set_compiler_warnings(imgui-dx11)
    target_sources(imgui-dx11 INTERFACE ${IMGUI_BACKENDS_DIR}/imgui_impl_dx11.h
...
if(IMGUI_IMPL_DX12)
  ...
  if(HAS_D3D11)
    ...
    imgui_set_compiler_warnings(imgui-dx12)
    target_sources(imgui-dx12 INTERFACE ${IMGUI_BACKENDS_DIR}/imgui_impl_dx12.h
...
if(IMGUI_IMPL_VULKAN)
  ...
  elseif(NOT Vulkan_FOUND)
    ...
    imgui_set_compiler_warnings(imgui-vulkan)
...
if(IMGUI_EXAMPLES)
    ...
    if(NOT
      ...
      if(WIN32)
        ...
        if(SDL_IS_IMPORTED)
          ...
          add_custom_command(
            TARGET ${IMGUI_EXAMPLE_TARGET}
            POST_BUILD
            COMMAND ${CMAKE_COMMAND} -E copy_if_different $<TARGET_FILE:${IMGUI_SDL_TARGET}>
                    $<TARGET_FILE_DIR:${IMGUI_EXAMPLE_TARGET}>)
        endif()
      endif()
    endif()
  endfunction()
  imgui_example(
    ...
    imgui-null)
  if(APPLE)
    # MacOS Metal example FIXME: This sample is still broken.
    add_executable(
      example_apple_metal MACOSX_BUNDLE
    ...
    string(
      REPLACE "$(PRODUCT_BUNDLE_IDENTIFIER)"
              org.imgui.example.apple-metal-macos
              INFO_PLIST
              "${INFO_PLIST}")
    string(
      REPLACE "$(MACOSX_DEPLOYMENT_TARGET)"
              10.12
              INFO_PLIST
              "${INFO_PLIST}")
    file(WRITE ${CMAKE_CURRENT_BINARY_DIR}/example_apple_metal-info.plist "${INFO_PLIST}")
    # Consume new plist
    set_property(TARGET example_apple_metal PROPERTY MACOSX_BUNDLE_INFO_PLIST
                                                     ${CMAKE_CURRENT_BINARY_DIR}/example_apple_metal-info.plist)
    # Compile storyboard
    if(NOT
       "${CMAKE_GENERATOR}"
       MATCHES
  ...
  if(CMAKE_SYSTEM_NAME STREQUAL Emscripten)
    ...
    target_link_options(
      ...
      -sASSERTIONS=1
      -sMAIN_MODULE=1)
    target_link_options(example_emscripten_opengl3 PUBLIC
                        "--shell-file=${IMGUI_EXAMPLES_DIR}/example_emscripten_opengl3/shell_minimal.html")
    if(IMGUI_DISABLE_FILE_FUNCTIONS)
      ...
      target_link_options(
        ...
        PUBLIC
        --no-heap-copy
        "--preload-file=${IMGUI_ROOT_DIR}/misc/fonts@/fonts")
    endif()
    # Uncomment next line to fix possible rendering bugs with Emscripten version older then 1.39.0
    # (https://github.com/ocornut/imgui/issues/2877) target_link_options(example_emscripten_opengl3 PUBLIC
    # -sBINARYEN_TRAP_MODE=clamp) target_link_options(example_emscripten_opengl3 PUBLIC -sSAFE_HEAP=1)    ## Adds
    # overhead TODO: Add example_emscripten_wgpu example
  else()
    imgui_example(
      TARGET
